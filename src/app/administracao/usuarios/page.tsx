  ...
  return (
    <AppShell>
      <RoleGuard allowed={["admin"]} fallback={<EmptyState title="Acesso restrito" description="Somente o administrador técnico pode gerenciar usuários." />}>
        <PageHeader title="Usuários" description="Consulta dos usuários cadastrados no Django Admin." />
        {users.isLoading ? <div className="rounded-app border border-border bg-white p-6">Carregando usuários.</div> : null}

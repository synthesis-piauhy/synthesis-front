...
export default function ReportsPage() {
  const { user } = useAuth();
  const reports = useQuery({ queryKey: ["activityReports"], queryFn: () => api.listActivityReports() });
  const users = useQuery({ queryKey: ["users"], queryFn: api.listUsers });
  const areas = useQuery({ queryKey: ["areas"], queryFn: api.listAreas });
  const cycles = useQuery({ queryKey: ["overview"], queryFn: api.getCollectionOverview });
  const open = cycles.data?.cycle.status !== "encerrada";
  ...
  return (
    <AppShell>
      ...
      />
      <div className="mb-4 max-w-xs"><WeekSelector /></div>
